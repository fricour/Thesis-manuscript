...
At 500 m, particles concentrations in the 102 - 645 $\mu$ range slowly decrease from July to October with a sharp decrease in November. Above 645 $\mu$m, the trend shows a maximum between the end of July and the first half of August though the concentrations of particles in those classes are very low (< 0.5 particle/L) on the entire timeserie.
...
particles_bis$wmo <- factor(particles_bis$wmo, levels = c(6904241,1902578,6904240,4903634))
...
particles_bis |> filter(depth < 2000) |>
  #group_by(size) |>
  ggplot() + facet_grid(wmo~size) + theme_bw() +
  geom_point(aes(x=juld, y=depth, colour=log10(rel_conc)), size=1, shape=15) +
  #geom_point(aes(x=juld, y=depth, colour=rel_conc), size=1, shape=15) +
  scale_colour_viridis(option = 'H', limits = c(-3,-1), oob = scales::squish, na.value = 'grey') +
  #scale_colour_viridis(option = 'H') +
  scale_y_reverse() +
  scale_x_date(labels = date_format("%m"), date_breaks = '3 month') +
  labs(colour = 'Normalized concentrations (#/L)', x = 'Month', y = 'Depth (m)') +
...
# # interpolate to see better
# dli <- particles_bis |> mutate(juldi = as.numeric(juld)) |>
#   group_by(wmo, size) |>
#   do({
#     i <- akima::interp(x=.$juldi, y=.$depth, z=.$concentration, nx=50, ny=150, duplicate="mean") |>
#       tidy()
#     # ggplot(i) + geom_raster(aes(x=x, y=-y, fill=z))
#     rename(i, juldi=x, pres=y, concentration=z)
#   }) |>
#   # re-compute relative concentration to see stuff better
#   group_by(size) |>
#   mutate(rel_conc=concentration/max(concentration, na.rm=TRUE)) |>
#   mutate(juld = as_date(juldi, origin = "1970-01-01")) |>
#   drop_na(rel_conc) |>
#   ungroup()
# 
# dli |> 
#   ggplot() + facet_grid(wmo~size) +
#   geom_raster(aes(x=juldi, y=-pres, fill=rel_conc)) +
#   #scale_y_reverse() |>
#   scale_x_continuous(expand=c(0,0), limits=c(NA, NA)) +
#   scale_y_continuous(expand=c(0,0), limits=c(-300,0)) +
#   scale_fill_viridis_c(option = 'H')


# test_sigma <- all_floats |> filter(PhaseName == 'NPAR', sigma > 26) |> select(juld, cycle, depth = pres, wmo, sigma, MLD) |> drop_na(sigma) |> mutate(juld = as_date(juld))
# test_sigma |> ggplot() + geom_point(aes(x = juld, y = depth, colour = sigma)) + 
#   scale_colour_viridis(option = 'H', oob = scales::squish, na.value = 'transparent') +
#   scale_y_reverse(limits = c(200,0)) +
#   scale_x_date(labels = date_format("%m"), date_breaks = '2 month') +
#   labs(x = 'Month', y = 'Depth (m)') +
#   facet_wrap(~wmo, nrow = 4) + theme_bw() +
#   geom_path(data = test_sigma, aes(juld, MLD), colour = "grey")
...
tmp <- uvp_data |> filter(park_depth != '200 m', month > 5) |> group_by(wmo, size, month, park_depth) |> 
  summarize(monthly_mean_conc = mean(conc), monthly_median_conc = median(conc)) |>
  # compute concentration anomaly at 500 and 1000 m
  mutate(anomaly = monthly_mean_conc - lead(monthly_mean_conc)) |> drop_na(anomaly) |>
  select(wmo, size, month, anomaly) |> mutate(month = month.abb[month]) |> 
  mutate(size = as.character(size)) |>
  #mutate(anomaly = round(anomaly, 3))
  ungroup()

tt <- tmp |> filter(wmo == 6904241) |> select(size, month, anom = anomaly)
#tt <- tt |> pivot_wider(names_from = month, values_from = anom) |> mutate(size = as.character(size))

x <- c('Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec')
y <- unique(tt$size)
data <- expand.grid(X=x, Y=y)
data$Z <- tt$anom

ggplot(data, aes(X, Y, fill= Z)) + 
  geom_tile() + scale_fill_gradientn(colors = c('darkblue', 'blue', 'white', 'red', 'darkred'))
