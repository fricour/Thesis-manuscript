...
UVP6 data and in particular the averaged number of particles per size class (i.e. PSD) was divided by the number of images at each depth and the water volume imaged by the UVP (0.7 L) to obtain particle concentrations (units: #/L).
...
### Derivation of carbon fluxes from the OST
...
```{r, UVP6 profiling mode without sinking speed small classes, fig.height=8, fig.width=10}
...
```{r, UVP6 profiling mode without sinking speed large classes, fig.height=8, fig.width=10}
