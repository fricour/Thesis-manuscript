...
### Phenology of the spring bloom in the Labrador Sea
...
```{r, spiciness}
#| label: fig-spice
#| fig-cap: Spiciness (up) and particle concentrations for the 102-128 $\mu$m size class.
...
It is also worth noting that the progressive emptying of the water column does not start from the surface in small classes [@fig-uvp6-profiling-102-406]. By computing the spiciness [@fig-spice], we show that this pattern does not likely result from water subduction (no spiciness anomalies) hence it would be support the hypothesis of the particle export ending.

All things considered, it is not straightforward to conclude for the presence of a carbon export event. First, the 10-day profiling configuration hinders the characterization of particle export events that are transient processes. Second, the plume of big particles can be misleading because their abundance measured by the UVP6 are very low (usually \< 0.1 #/L). Third, all floats measured the same particle dynamics in distinct zones which suggests that if the two systems were initially different (i.e. different bloom dynamics), they merged back after the bloom of May. Therefore, we suggest that we mostly observe the latent dynamics of the system rather than clear export signatures that would likely be spotted with both higher bloom intensity and a higher profiling temporal resolution.

### hplc pour les communautés pour montrer que les deux endroits sont différents (peripery vs central)

mettre ça dans phenology of the bloom pour bien montrer que l'on est dans 2 systèmes différentes

```{r, HPLC}

pigments <- vroom('DATA/ARGO/CE22009_pigments_031122_Travail_Herve_FŔ.csv') |> filter(Station %in% c(29,9))

# select interesting coumns (all QCs == 1 so OK)
# pigments <- pigments |> select(station = Station, depth = `Depth_(m)`, Chla_c3 = Chlorophyll_c3, Chla_c1_c2_MgDVP = `Chlorophyll_c1+c2+MgDVP`, Phaeophorbid_a, Peridinin, `19'-Butanoyloxyfucoxanthin`
```
