...
all_floats <- all_floats |> dplyr::mutate(wmo = factor(wmo, levels = c(6904240,4903634,6904241,1902578)),
...
basemap(limits = c(-55,-40,55,65), bathymetry = T, lat.interval = 2.5, bathy.style = 'poly_blues',
        grid.col = 'black') +
  ...
  guides(color = guide_legend(override.aes = list(size = c(1.5,1.5,1.5,1.5),
                                                  shape = c(NA, NA, NA, NA)))) #+
  # basemap(limits = c(-55,-40,55,65), bathymetry = F, lat.interval = 2.5,
  #       grid.col = 'black')
...
#| label: fig-OST-flux
#| fig-cap: OST-derived small (top) and big (bottom) particles carbon flux for all floats at each drifting depth. Black lines and shaded areas represent, respectively, the monthly mean flux and the 95% confidence interval around the mean. 
...
tmp <- uvp_data |> filter(park_depth != '200 m') |> group_by(wmo, size, month, park_depth) |>
  ...
  dplyr::mutate(diff_anomaly = monthly_mean_conc - lead(monthly_mean_conc), div_anomaly = monthly_mean_conc/lead(monthly_mean_conc)) |> drop_na(diff_anomaly) |> #filter(is.finite(div_anomaly)) |>
  select(wmo, size, month, diff_anomaly, div_anomaly) |> dplyr::mutate(month = month.abb[month]) |>
  dplyr::mutate(size = as.character(size)) |>
  dplyr::mutate(rev_div_anomaly = 1/div_anomaly) |>
  #filter(is.finite(rev_div_anomaly)) |>
  dplyr::mutate(diff_anomaly = round(diff_anomaly, 1), rev_div_anomaly = round(rev_div_anomaly, 1), div_anomaly = round(div_anomaly, 1)) |>
  ungroup()
...
#   dplyr::mutate(rev_div_anomaly = 1/div_anomaly) |> 
#   #filter(is.finite(rev_div_anomaly)) |>
#   dplyr::mutate(diff_anomaly = round(diff_anomaly, 1), rev_div_anomaly = round(rev_div_anomaly, 1)) |>
#   ungroup()
# 
...
dd <- vroom('heatmap_data.csv') |> dplyr::mutate(Y = as.character(Y)) 
dd$X <- factor(dd$X, levels = c('May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec', 'Jan', 'Feb', 'Mar'))
...
#dd <- dd |> dplyr::mutate(Z_binned = cut(Z, breaks = c(0,1,1.5,2.5,max(Z))))
dd <- dd |> dplyr::mutate(Z_binned = cut(Z, breaks = c(0,.5,1,1.5,2,max(Z))))
