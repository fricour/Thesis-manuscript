...
parking_data <- parking_data |> mutate(rainer_flux = compute_flux_rainer(2.8649, 2.24, parking_spectra_rainer)) 
parking_data <- parking_data |> mutate(iversen_flux = compute_flux(273, 4.27, parking_spectra)) 
...
get_jump_info <- function(data){
  ...
  # clean data to send
  slope_df <- slope_df |> arrange(min_time) |> mutate(min_drifting_time = df$min_time, max_drifting_time = df$max_time, small_flux = df$small_flux, big_flux = df$big_flux, park_depth = data$park_depth[1], wmo = data$wmo[1],cycle = data$cycle[1])
  if(is.null(tmp3)){
    big_jump <- tmp3
  }else{
     big_jump <- tmp3 |> mutate(park_depth = data$park_depth[1], wmo = data$wmo[1],cycle = data$cycle[1]) 
  }
...
uvp_drifting <- all_floats |> filter(PhaseName == 'PAR') |> select(juld, cycle, wmo, park_depth, all_of(lpm_class)) |> 
  drop_na(NP_Size_102) |> mutate(date = as_date(juld)) 
...
uvp_drifting <- cbind(uvp_drifting, spectra_biovolume) |> rowwise() |> mutate(total_biovolume = sum(across(all_of(biovolumes_names))))
...
for (i in wmo){
  ...
  for (j in park_depth){
    for (k in seq(1:max_cycle)){
      tmp_uvp <- filter(uvp_drifting, wmo == i, cycle == k, park_depth == j) |> arrange(juld) |> select(juld, cycle, wmo, park_depth, total_biovolume) #, biovolumes_names)
      tmp_ost <- res_slope |> filter(wmo == i, cycle == k, park_depth == j)
      tmp <- assign_time_group(tmp_uvp, tmp_ost)
      #tmp2 <- tmp |> filter(!is.na(group)) |> group_by(group) |> mutate(uvp_points = n(), min_date = min(juld), max_date = max(juld), mean_date = mean(floor_date(juld))) # https://stackoverflow.com/questions/44814435/calculate-average-daily-value-from-large-data-set-with-r-standard-format-date-ti
      tmp2 <- tmp |> filter(!is.na(group)) |> group_by(group) |> mutate(uvp_points = n(), mean_date = mean(floor_date(juld))) 
      tmp3 <- merge(tmp2, tmp_ost) |> group_by(group, cycle, wmo, park_depth) |> summarize(mean_date = unique(mean_date), uvp_points = unique(uvp_points), ost_points = unique(nb_points), group_total_biovolume = sum(total_biovolume), slope = unique(slope), weighted_group_biovolume = group_total_biovolume/uvp_points)
...
# check
test2 <- test |> group_by(wmo, cycle) |> mutate(mean_mean_date = mean(floor_date(mean_date))) 
...
ggplot(test) + geom_point(aes(x = log(slope), y = log(weighted_group_biovolume))) + facet_wrap(~park_depth)
...
uvp_drifting <- uvp_drifting |> mutate(num_date = as.numeric(juld))
res_jumps <- res_jumps |> mutate(num_date = as.numeric(dates))
...
ggplot(test_jump) + geom_point(aes(log(diff_jump), log(biovolume)))
```
...
# # get biovolume for each DSE (equivalent to say for each size bin)
# biovolumes <- map_dbl(DSE, get_biovolume)  # unit = mm^3
...
ggplot(test, aes(x = ztime, y = fgauss, colour = factor(depth))) + geom_point() + theme_bw() #+ ylim(c(0,.2)) 
