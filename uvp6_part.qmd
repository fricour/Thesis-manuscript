...
Building the zooplankton classification model for the UVP6 was the first task of my PhD. I knew nothing about machine learning before I started, so I spent the first three weeks reading a machine learning book to get familiar with the concepts. To be even more honest, I also knew nothing about zooplankton either, so there was something else to learn.

In machine learning, the "no free lunch theorem" [@Wolpert1996-hw] states that if you do not make any assumption about your data, then there is no model that is a priori guaranteed to work better [@Geron2019-is]. In other words, it means that you need to get to know your data and your hands dirty to find a model that works. The first step consisted in developing a full data processing pipeline for UVP5 images. The aim was to be prepared for the UVP6 image data set (which was not ready at that time) to train the true, final, classification model. To this end, I developed a python package called [UVPec](https://github.com/ecotaxa/uvpec) that is able to create a full classification model based on image folders with labeled particle/zooplankton images and some hyperparameters (e.g. learning rate). It made my life easier when I got the final UVP6 data set and should serve other future users in the same way.

Classifying zooplankton on UVP images is difficult for human operators so building a model to automatically classify them is already a challenge, but adding technical and energy constraints to it was the icing on the cake. Below is the first draft of a publication on the classification model.
...
Millions of images of oceanic particles have been acquired with ship-based imaging instruments. The identification of objects on images is time consuming for human operators but can be accelerated using artificial intelligence algorithms. Autonomous Underwater Vehicles (AUVs) such as Biogeochemical-Argo floats (BGC-Argo) have revolutionized our understanding of oceanic systems but have limited battery life and must spend as little time as possible at the ocean surface to transmit their data, which means that images cannot be transmitted. In this paper, we describe an image classification model embedded in the Underwater Vision Profiler 6 (UVP6) that is appropriate to be run on the limited power and bandwidth of AUVs. To this end, we computed simple global features of images of objects of interest and gradient boosted trees as a classifier. We compared its performance with a state-of-the-art convolutional neural network, which is much more energy demanding. In addition, we show how we dealt with class imbalance (\> 80% of images are marine snow) and what were the compromises to improve the classification scores of biological classes. Finally, we show the classification report (precision and recall for each label) of the final classification model and use it to analyze preliminary results from BGC-Argo floats deployed in the Labrador Sea.

### Introduction

Inside the ocean, zooplankton constitutes a cohort of very diverse organisms. They are important enough for the functioning of our oceans [@Steinberg2017-ri] to motivate the development of a large number of instruments dedicated to taking pictures of them [@Picheral2010-to; @Gorsky2010-ca; @Cowen2008-wq; @Benfield2007-lt; @Lombard2019-ld]. The amount of images taken by those instruments has quickly become so large that it is now unfeasible to classify them manually, through the observation of human operators. Fortunately, powerful machine learning techniques have been implemented to tackle the issue of automatic plankton image classification [@Luo2018-oo].

However, it remains a fierce challenge. Within a single taxon, individuals with very different sizes and shapes coexist, due to significant morphological changes during ontogeny, imaging of portions of organisms only (e.g. siphonophores) or organisms taken from various angles. This variability hinders the performance of automatic classification [@Benfield2007-lt]. In addition to this global heterogeneity, for in situ instruments, the large majority of images are not of zooplankton: non-living particles (mostly marine snow) largely dominate, resulting in highly imbalanced data sets.

Automatic plankton classification is an active research topic. Within this context, the application of Convolutional Neural Networks (CNNs) has already lead to promising results [@Luo2018-oo; @Cui2018-nt; @Dai2017-ex; @Campbell2020-cq; @Schroder2019-um; @Bochinski2019-af; @Lee2016-jl; @Cheng2019-ar; @Guo2021-xi; @Ellen2019-nh; @Orenstein2017-ya]. Indeed, CNNs have outperformed the previous features-based classifiers (i.e. machine learning algorithms using a set of features extracted from the image such as area, eccentricity, gray levels, etc.) [@Ellen2019-nh]. Nevertheless, real-time image processing, performed at sea, in the instrument, has not been tackled, to our knowledge. Usually, images are brought back to the lab and classified with classic machine learning classifiers and, increasingly, CNNs. But CNNs are energy consuming, even just for inference (i.e. processing and classification of new images, without retraining), and their implementation for embedded applications can be limited by power consumption [@Biswas2018-uw; @Khabbazan2019-oi; @Oh2017-kj].

This is of particular importance for autonomous underwater vehicles (AUVs) and especially for a new version of Biogeochemical-Argo (BGC-Argo) floats that will be equipped with the Underwater Vision Profiler 6 (UVP6), the portable version of the UVP5 [@Picheral2010-to] to better understand the carbon cycle [@Xing2018-zo]. The lifetime of floats is limited by their batteries hence by the power consumed by its sensors. The floats are not recovered and their data has to be sent through satellite communication. Therefore, using an embedded CNN to classify plankton images onboard or sending images through the satellite link are both impossible with currently available hardware. [JOI: ici il manque une justif de pq c'est poas possible de mettre un CNN dans le flotteur; il y a qq chiffres dans l'étude de FOTONOWER; tu peux aussi parler des cartes dédiées mais qui sont encore largement trop consommatrices => il manque un chiffre sur la conso énergétique en W attendue d'un capteur; un ordre de grandeur suffit]. Typically, BGC-Argo floats sample the water column between 1,000 m [JOI: c'est pas 2000?] and the surface; in this range, the UVP6 is expected to take images of hundreds of planktonic objects and, given the bandwidth of the current connection, it would require the float to stay [JOI: n minutes/hours/days] to transmit them [JOI: pareil, on avait des stats et même des estimations de temps, il faut donner les chiffres], consuming energy proportionally to the amount of data sent and being subject to biofouling or unpredictable hazards. For these reasons, it was decided to use a simpler features-based classifier for the development of the embedded plankton recognition algorithm on board the UVP6.

Such algorithms (e.g. Decision Tree (DT), Random Forest (RF), Support Vector Machine (SVM)) have already been used for plankton classification (see Table 1 of @Gonzalez2017-tr for an exhaustive list at that time). For instance, @Blaschko2005-ep obtained an accuracy of 71% using a SVM on a set of 982 images among 13 classes using a 10-fold cross-validation (CV), @Sosik2007-cj obtained a global accuracy of 88% using a SVM on a test set of 3300 images covering 22 classes, @Bell2008-yo obtained 82% of accuracy using a 10-fold CV RF algorithm on 53 classes with 10 to 30 images per class. However, it should be noted that this accuracy dropped to 63% after the removal of non-biological classes such as detritus and fiber. Using a larger data set or 650.000 images, similar to the one used in this study, @Orenstein2017-ya trained a RF model over 70 classes that yielded an accuracy of 91% on the test set (2.75 million images). Though, one class in that test set contained 1.6 million images and a dummy classifier that would have put every object in that class would have given an accuracy of about 60%; so caution must be applied when interpreting this global performance metric. @Faillettaz2016-wr trained a RF model on 6000 images across 14 classes, reaching an accuracy of 56% on a test set of 1.5 million objects, discarding those below a certain probability threshold. This allowed to improve the global accuracy by 16% but resulted in the loss of 72% of objects in the predicted data set. [JOI: Il y a une review par Irisson et al qui fait un petit bilan de tout ça ;-)]

It is quite difficult to compare all methods due to the diversity of imaging instruments resulting in a variety of image resolutions, numbers of classes (while there is a trade off between model performance and the number of classes as illustrated in @Fernandes2009-hw), training sets (e.g. number of images per class, diversity, balance), scientific objectives (e.g. precision-recall trade off), types and numbers of features used and classification algorithms. In many cases, RF seemed to be the best classifier [e.g. @Gorsky2010-ca] [JOI: je crois que c'est plutôt grosjean]. For embedded applications, the number of parameters in the model need to be small for its memory footprint to be reduced and the computation of new classifications should be fast. Gradient Boosted Trees (GBT) seem to offer this compromise when compared to RF (model size) and SVM (computation complexity). For these reasons, we chose to classify images using the XGBoost library [@Chen2016-xt], an optimized implementation of GBT, known to be high performing in machine learning competitions (on the Kaggle website for example).

In this paper, we present the results of an embedded zooplankton classification algorithm using XGBoost on UVP5 data. We show how we built the embedded classification (EC) [JOI: UVPec = UVP embedded classification => soit cohérent; et puis recognition c'est pas trop ça en Anglais] model under strict technical constraints, what were our compromises and what techniques we used to improve the classification. We also compare the performances of our features-based algorithm with a state-of-the-art neural network as if energy was not a limiting factor. Finally, we apply the method and follow the recommendations learnt from the preliminary UVP5 model to the UVP6 data set to build the EC model that will be included in the UVP6 for AUVs.

### Material and method

#### Instruments

The UVP6 is the new version of the UVP5 [@Picheral2021-sx]. It was made smaller and more energy efficient specifically to be mounted on AUVs such as BGC-Argo floats, while keeping the aim of providing data of quality similar to that from the UVP5. There are 2 versions: the LP (low power) and the HF (high frequency). The former uses less energy (lower image acquisition rate) and will be deployed on AUVs while the latter is designed to be used on CTD rosettes and cruising AUVs [@Picheral2021-sx].

UVP5 Standard Definition (SD) profiles have been collected since 2008 at a global scale (@fig-map-of-profiles) whereas the UVP6 has only been deployed very recently, in few locations and the resulting dataset lacks spatial coverage to be fully representative. Yet, the images of the two instruments have a similar aspect and they target the same size range. For this reason, UVP5 data will be used to create a preliminary EC model and explore ways for improvement. Ultimately, we will train the EC model on the growing UVP6 data, by applying the lessons learnt from the preliminary UVP5 model.
...
The UVP5 SD data set is composed of 3.578.901 validated images whereas the full UVP6 LP data set has 634.459 validated images. Each image was segmented to find the main object (i.e. the biggest connected region in pixels). From it, 55 features were extracted to describe this object in terms of shape (e.g. pixel area, eccentricity), gray levels and statistical moments (see @tbl-features). For technical reasons, all UVP6 images with a pixel area below 79 pixels were removed (7% of images).

```{r, features, dpi = 300}
#| label: tbl-features
#| tbl-cap: List of 55 features (abbreviations used in our python package UVPec (https://github.com/ecotaxa/uvpec) and their descriptions) extracted from each object based on their geometry and gray levels.
...
#| tbl-cap: Precision, recall and f1-score for each class and globally in the UVP5 model (XGBoost and CNN). The abbreviation avg stands for average.

labels <- c('Acantharia', 'Aulacanthidae', 'Aulosphaeridae', 'Copepoda', 'Trichodesmium', 
            'artefact', 'bubble', 'darksphere', 'detritus', 'fiber', 'other<living', 'accuracy',
...
GBT generates a sequence of trees, each one trying to cope with the residual error from its predecessors. XGBoost is an optimized and regularized version of GBT. To learn from the data and derive a model, XGBoost first minimizes a regularized objective function $L$
...
To build the EC model, the first step was a threefold cross-validation using XGBoost parameterized with a set of given hyperparameters (i.e. learning rate ($\eta$), maximum depth of a tree ($D_{max}$) and weight sensitivity ($\omega$)). The second step consisted in the search of the optimal number of boosting rounds (i.e. $k$) before the model overfits. For this, we searched for the minimum value of the multiclass log loss (also known as cross-entropy loss) evaluated at each boosting round. Using that optimal $k$ value, we then trained the EC model with the entire training set. Therefore, finding the best model consisted in finding the set of hyperparameters ($k$, $\eta$, $D_{max}$) that respected the energy constraints and returned the best global metrics. The latter were determined using a customary grid search (@fig-grid-search) while $\omega$ was chosen empirically (see hereafter).
...
To cope with our unbalanced data set (more than 70% of detritus in the UVP5 data set), several options were available. We could artificially increase the number of biological data with the common Synthetic Minority Oversampling Technique (SMOTE) [@Chawla2002-kg; @Fernandez2018-ft], decrease the number of detritus (random undersampling, keep only detritus close to unsupervised cluster centroids, etc.) or add a weight to each label during training to nudge the model towards under-represented classes (i.e. biological classes). After testing the various techniques, we chose the weight-based one, which also has the advantage or being directly comparable to the same implemntation in CNNs, and we defined the weight of a class as follows:

$$
class~weight = \left(\frac{size~max}{size~class}\right)^{\omega}
$$ {#eq-class-weight}

where $size~max$ and $size~class$ are, respectively, the size (number of examples) of the most numerous class (i.e. detritus) and the size of each class. Hence, the weight of detritus is 1 and all other classes have a weight \> 1.

We decided to select the best $\omega$ among 0 (no weights), 0.25, 0.50 and 0.75 (model very biased towards biological classes). In order to choose between all options, we compared the predicted concentrations of classes for each $\omega$ with the real concentrations of each class in different regions of the global ocean. As always in machine learning, we encountered a trade off between precision and recall. However, given that the UVP6 is more focused on carbon flux estimation (high precision) than on the detection of rare objects (high recall with low precision), a weight sensitivity of 0.25 seemed to be the best compromise.

In order to compare our features-based model with a state-of-the-art approach, not limited by energy constraints, we classified the datasets with a Convolutional Neural Network (MobileNetV2 , depth multiplier 1.40), pre-trained on ImageNet. This is known as transfer learning and it was already applied with success for zooplankton classification in @Orenstein2017-ya. Input images have a size of \[224, 224, 3\], the size of the features extractor vector is 1792 followed by a fully connected layer (600 nodes) with a dropout of 0.4 to avoid overfitting. The classification layer at the end of the network also has a dropout of 0.2. The CNN was trained with a decaying learning rate to facilitate a stable convergence towards a minimum of the loss. Finally, we used settings comparable to the GBT: a categorical cross entropy loss function and weights defined with the same weight sensitivity of 0.25. The CNN was trained with a sufficient number of epochs such that its training reached a validation plateau (i.e. the model was not learning much with additional epochs).

### Results

It is well known that the task of labeling images in supervised machine learning tasks is time consuming and most probably painful when you have millions of images to label. It is also very frustrating to see that one class (largely) dominates when you are interested in the small ones and that some of them are so scarce that you need to merge them with their siblings (at the very best) on the phylogenetic tree. Overall, we found - after multiple trials and errors - 11 classes fulfilling a first compromise between classification performances and ecological importance: 7 biological classes (*Acantharia*, *Aulacanthidae*, *Aulosphaeridae*, *Copepoda*, *Trichodesmium*, darksphere and other\<living) and 4 non-living classes (detritus, bubble, fiber and artefact).

Once the number of classes is known, hyperparameters can be tuned to find the optimal set for the classification results (i.e. grid search results shown in @fig-grid-search). In our case, the optimal model hyperparameters were 0.2 for $\eta$ and 5 for D$_{max}$ (also with only 1 class of detritus, see Discussion) to have both a good weighted living (or biological) precision and recall and a loading time (i.e. loading of the EC model in memory) below 700 ms for energy reasons. In that configuration, the UVP5 model had an accuracy of 81%, a macro (i.e. label imbalance not taken into account) precision and recall of, respectively, 64% and 69% and a weighted living precision and recall of, respectively, 60% and 65%. The CV found an optimal number of 850 boosting rounds before overfitting and the expected loading time is 382 ms.

The classification report (CR) gives the precision, the recall, the f1-score (harmonic mean of the precision and recall) and global metrics on the test set. As it can be seen in @tbl-classification-report-uvp5-xgboost-cnn, detritus have a recall of 86% thus because detritus are abundant (more than 230.000 in the test set), it results in more than 30.000 detritus wrongly labeled and therefore disseminated in all other classes. The confusion matrix (CM) helps to see where those wrongly labeled images are. @fig-confusion-matrix-uvp5 shows that a lot of Copepoda are confused with detritus as well as fiber. The worst class in the CR is other\<living, which is not surprising because this class is composed of organisms that are not all morphologically and ecologically related. The best biological class is Aulosphaeridae with a f1-score of 82%.
...
#### Ways to improve the classification {#sec-improve-classif}
...
kable(cf_uvp6_xgboost_cnn) |> row_spec(0, bold=T, align = 'c') |> add_header_above(c(" " = 1, "XGBoost" = 3, "CNN" = 3))
```
...
4 BGC-Argo floats equipped with the EC were deployed in late May 2022 in the southern part of the Labrador Sea at the time of the spring bloom. As it can be seen in @fig-bgc-application-uvp6, *Calanoida* are more abundant around the deep chlorophyll maximum (DCM) and they might be observed in very low concentrations even below 1000 m. The CR for *Calanoida* shows that the model has a precision of 66% and a recall of 69% for that class. It implies that caution must be applied when interpreting the presence or absence of such copepods. In that case, we theoretically miss 31% of them (i.e. 31% of *Calanoida* are not detected as such, they are classified in another class) and out of the 69% that we detect as *Calanoida*, we are wrong 34% of the time. As a result, even though *Calanoida* have been shown to overwinter in deep waters (\> 1000 m) [@Krumhansl2018-dr], an isolated signal at depth does not mean a copepod was really there. Similarly, *Trichodesmium* should not be observed outside the tropical and subtropical regions [@Westberry2006-bt] therefore depending on the sampling region, some results can directly be rejected. Here, tuff are predicted in higher concentrations than puff which are only predicted at one depth. Again, if one looks at the CR for these 2 classes, one can see that the precision and recall for tuff are, respectively, 18% and 15% below those of puff. Finally, the interpretation of the detritus class is more straightforward because they have excellent metrics in the CR. For instance, it can be seen that detritus nicely follow the DCM curve. In contrast with biological classes where we are looking for patterns of presence or absence, we know that detritus are ubiquitous but their successive abundances could be interesting for carbon flux studies.

```{r, uvp6 BGC application, fig.width=6, fig.height=5}
#| label: fig-bgc-application-uvp6
#| fig-cap : Concentrations of 4 classes (3 biological and the detritus class) estimated with data acquired from the UVP6 on board the BGC-Argo float (WMO 4903634) on the 15$^{th}$ of June 2022 in the Labrador Sea. Red points represent positive concentrations while blue points represent the absence of the specified class. Dotted lines represent the depth of the deep chlorophyll maximum.
...
chla <- ggplot(tmp, aes(x = pres, y = mean_conc)) + geom_point() + scale_x_reverse() + coord_flip() +
  xlab('Depth (m)') + ylab(expression('Chlorophyll a (mu/L)')) + theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank()) 
...
cal <- ggplot(test, aes(x = Pressure_dbar, y = mean_conc)) + geom_point(colour = "#005CAB") + scale_x_reverse() + coord_flip()+
  xlab('Depth (m)') + ylab('Calanoida (#/L)') + geom_point(data = test[test$mean_conc>0,], 
                                                                  aes(x = Pressure_dbar, y = mean_conc), colour = "#E31B23") +
...
det <- ggplot(test, aes(x = Pressure_dbar, y = mean_conc)) +  geom_point(colour = "#005CAB") + scale_x_reverse() + coord_flip() +
  geom_point(data = test[test$mean_conc>0,], 
             aes(x = Pressure_dbar, y = mean_conc), colour = "#E31B23") +
  xlab('Depth (m)') + ylab('detritus (#/L)') + theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
  geom_vline(xintercept = DCM, lty = 'dotted') + theme_bw()
...
puf <- ggplot(test, aes(x = Pressure_dbar, y = mean_conc)) +  geom_point(colour = "#005CAB") + scale_x_reverse() + coord_flip()+
  xlab('Depth (m)') + ylab('puff (#/L)') + geom_point(data = test[test$mean_conc>0,], 
                                                                          aes(x = Pressure_dbar, y = mean_conc), colour = "#E31B23") +
  geom_vline(xintercept = DCM, lty = 'dotted') + theme_bw()
...
tuf <- ggplot(test, aes(x = Pressure_dbar, y = mean_conc)) +  geom_point(colour = "#005CAB") + scale_x_reverse() + coord_flip()+
  xlab('Depth (m)') + ylab('tuff (#/L)') + theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
  geom_point(data = test[test$mean_conc>0,], 
             aes(x = Pressure_dbar, y = mean_conc), colour = "#E31B23") +
  geom_vline(xintercept = DCM, lty = 'dotted') + theme_bw()
...
The EC of zooplankton on board AUVs is just at its beginning. As expected, the CNN trained on UVP6 data (see @tbl-classification-report-uvp6-xgboost-cnn) outperforms the results of our features-based classifier. However, it should be noted that not all biological classes are better classified (e.g. *Actinopterygii*, *Appendicularia*, *Chaetognatha*, *Creseis*). One can also observe that the difference between XGBoost and the CNN trained on UVP5 data on biological classes is less significant than for UVP6 data. This is likely explained by the fact that we were able to keep more (smaller) classes for the UVP6. Nevertheless, previous tests on UVP5 data with more classes increased the gap between XGBoost and CNN weighted results therefore the latter better recognizes small classes. If energy and hardware constraints limit the performances of our current EC model, it is not forbidden to think about light-weight CNNs for future applications on AUVs to better understand the dynamics and the behavior of zooplankton.

![Scheme of the UVP6 pipeline with the processing done in the lab to create the EC model and the processing time (in ms) of the main steps when the UVP6 is deployed on board an AUV. kpx means kilo pixel.](image/uvp6/fig_pipeline_UVP6.jpg){#fig-uvp-pipeline}

### Conclusion

The objective of this paper was to develop a zooplankton classification model under strict energy constraints for the UVP6 to be deployed on BGC-Argo floats. It was clear at the beginning that the use of powerful energy consuming CNNs had to be disregarded and that we needed to build a satisfactory features-based model instead. To this aim, we firstly used XGBoost on UVP5 data, a more spatially and temporally representative data set than the recent UVP6 data set to create the data processing pipeline (class aggregation, hyperparameter tuning, model training, see also @fig-uvp-pipeline) and investigate ways to improve classification scores (quantification, detritus subgroups, label weights). In order to see how the model could be improved, we compared the XGBoost model with results from a CNN not limited by energy. Afterwards, we applied the data processing pipeline and the recommendations from the UVP5 data set analysis on the UVP6 data set to create the final EC model. This model contains 20 labels, 16 biological and 4 non-living classes, with a total loading time of 73 ms. The initial upper limit for the loading time was 700 ms therefore we are saving precious energy to extend the lifetime of BGC-Argo floats. Finally, we showed the CM and CR of the final model and how to use it to interpret some UVP6 taxonomic profiles observed by the float WMO 4903634 during the spring bloom in the southern part of the Labrador Sea.
