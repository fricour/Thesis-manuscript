...
UVP5 Standard Definition (SD) profiles have been collected since 2008 at a global scale (@fig-map-of-profiles) whereas the UVP6 has been deployed very recently in specific locations and thus lacks spatial coverage to be fully representative. For this reason, UVP5 data will be used to create a preliminary ER model and explore ways of improvement. Ultimately, we will train the ER model for the UVP6 by applying the lessons learnt from the preliminary UVP5 model.
...
The full UVP5 SD data set is composed of 3.578.901 validated images whereas the full UVP6 LP data set has 634.459 validated images. Each image was segmented to find the main object (i.e. the biggest connected region in pixels). From it, 55 features were extracted to describe this object in terms of shape (e.g. pixel area, eccentricity), gray levels and statistical moments (see @tbl-features). For technical reasons, all UVP6 images with a pixel area below 79 pixels were removed (7% of images). 
