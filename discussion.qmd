...
The last technical limitation was the number of features (see @tbl-features). Before I started my PhD, 55 features (basically geometry, gray levels, statistical moments) were defined. I was not allowed (although, I asked!) to do some feature engineering, that is either doing feature selection (selecting the most useful features) either doing feature combination (combining existing features to create new ones) or creating new features by collecting new data [@Geron2019-is]. Why was this proposition refused? Again, the 55 selected features had been tested earlier and they were compliant with the energy budget. Therefore, adding more features was not a possibility because it would have required retesting them and reevaluting energy budgets that were not possible at the time. In addition, all 55 features were computed quickly hence they were not consuming a lot of energy so it was decided to simply keep them. At worse, some features would have been redundant, without impacting the classification quality.
...
We have seen with CONVERSE that carbon sequestration occurs on the whole water column and that using a fixed reference depth would likely miss the bulk of sequestration. Therefore, the evaluation of future mCDR strategies using biologically-driven carbon sequestration fluxes should be estimated over the whole water column.

### Particle injection pumps

Our study on the sequestration flux by the BCP also shows that the contribution of the vertical migration and mixing pumps contribute at least to 20-31% of the sequestration flux by the BCP.

dire qu'il faut améliorer la physical et la VMP (diel et saisonnière) -\> mettre le paquet sur ARGO (et les gliders pour la physical) + papier d'Hervé de 2021. + préciser qye la migration saisonnire est sous-évaluée.
