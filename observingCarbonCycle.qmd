...
Because measuring directly the POC export flux would be challenging, it is common to estimate it as the fraction of the total NPP that is exported (i.e. export efficiency) [@Henson2011-mt].

::: {.blackbox data-latex=""}
::: {#box-npp .center data-latex=""}
A deeper look into net primary production
:::

Total net primary production includes new production (NP, mainly fueled from the upwelling of nitrogen from deeper water to the EZ) and regenerated production (RP, fueled with nitrogen recycled through the food web within the EZ) \[@Dugdale1967\].

@Eppley1979 introduced the f-ratio as follows

$$
f-ratio = \frac{NP}{NP+RP}
$$

In a steady state system, new production equals export production therefore the $f-ratio$ should be equal to the ratio of export production (EP) to total NPP, referred to as the $e-ratio$ by @laws2000-XX.
...
Empirical models likely represent the most straightforward option to estimate carbon export.

combining satellite observations and in situ measurements

Data-assimilated models

These models can either target the estimations of carbon export

These models (i.e. food-web, empirical and inverse models) incorporate satellite data (@sec-observation-tools), that can be defined as primary data product

numerical models based on simple empirical relationships to derive primary and secondary data products

As illustrated in @fig-bgc-models,

One of the key variable to estimate carbon export is the NPP. It has been used to compute the f-ratio, e-ratio -\> rentrer un peu là-dedans

The former

Henson2010

-   common approach = estimate fraction of total primary production that is exported (with f-ratio, Th234-ratio

-   encadré sur le f-ratio? en lien avec laws2000 ry Dugdale and Goering 1967

-   satellite data

Dunne2007

-   NPZD models (non exhaustive)

-   empirical models: predictor variables

-   empirical algorithm

Henson2011

-   Martin b : -0.86 (6 stations, in the NOrtheast Pacific), lack of data so b is generally assumed to be spatially constant and this b value has been applied in several biogeochemical models to describe the rate of particle reminrelaiation in the water column

-   data-derived algorithms

Siegel 2014

-   food-web model (e.g. NPZD)

-   estimation og floval exports by empirical extrapolations of field measurements or by using global coupled carboncyckle-pcean circualtion models but estimations of global export remain widely divergent

-   mechanisticlly, it is the flux of sinking carbon leaving the EZ that is relevant

-   e-ratio ? different du f-ratio?

-   satellite-based approach for assessing carbon export (thank you kostadinov 2009 and 2010) -\> slope of the particle size spectrum

-   mechanistic approach for estimating global carbon export using satellite observations and a food-web model

-   a complete assessment of carbon export from the surfce ocean must accout for this and other additional pathways

-   the BCP is more than just the export of carbon from the EZ. Understanding it, also requires the quantification of the vertical attenuation of the export flux below the EZ as well.

-   aller absoilument checker siegel 2022

Devries2017

Note perso

Ajouter Guidi 2015 pour une estimation de b regionale
...
Remote sensing data acquired with ocean color satellites have been used for a long time [@Gordon1980-dm] to study key EZ variables related to carbon export [@Claustre2021-tx] such as phytoplankton biomass [@Robinson2004-cj], phytoplankton communities [@Uitz2015-hd] and particle size distribution [@Kostadinov2009-gf; @Kostadinov2010-bu] to name a few. While having a wide spatial coverage, these platforms are limited to the ocean's surface and their measures can be affected by several factors such as the cloud cover or low solar elevation (e.g. @Marshall1994-vk).
...
In allThe study of the BCP is a really hot topic in oceanography with new platforms (e.g. unmanned surface vehicles such as saildrones or wave gliders) and new technologies (e.g. lidar) being continuously developed in the hope of understanding all its components [@Chai2020-uq; @Claustre2021-tx].

## Specific sensors for the study of the BCP {#sec-BGC-sensors}

In the frame of the REFINE (Robots Explore plankton-driven Fluxes in the marine twIlight zoNE) project, enhanced BGC-Argo floats (@fig-bgc-float) will have two new sensors specifically designed for the study of the BCP: a transmissometer and the Underwater Vision Profiler 6 (UVP6) [@Picheral2021-sx].

![Illustration of a BGC-Argo float used in the frame of REFINE. From top to bottom: GPS & Iridium (two-way communication) antenna, Oxygen optode, CTD (salinity, temperature and depth), pH sensor, radiometer (downwelling photosynthetic available radiation and downwelling irradiance at 380, 412 and 490 nm), fluorometer (Chla and colored dissolved organic matter) and scattering meter ($b_{bp}$), UVP6 (particle size spectra and images), transmissometer (particle beam attenuation) and nitrate sensor. \@Thomas Boniface.](image/ObservingCarbonCycle/float_picture1.png){#fig-bgc-float width="500"}
...
The value of $c$ is the result of absorption and scattering by particles, water and dissolved substances [@Zaneveld1994-ez]. The transmissometer carried by REFINE BGC-Argo floats uses a light beam at 660 nm whose attenuation coefficient can be splitted into three components: the absorption and scattering of light by particles ($c_{p}$), the absorption by water ($c_{w}$) and the absorption by dissolved organic matter ($c_{DOM}$) [@Jerlov1976-hr]. At 660 nm, $c_{w}$ is assumed constant at 0.364 m$^{-1}$ and $c_{DOM}$ is considered negligible [@Jerlov1976-hr] hence variations of $c$ will be assumed to be variations of $c_{p}$ in what follows.

The use of transmissometers to characterize POC concentrations was shown by @Bishop1999-es who found that POC concentrations were well correlated with $c_{p}$, independently of ocean environment, season or depth. Following that, the idea of using a transmissometer as an Optical Sediment Trap (OST) was firstly applied by @Bishop2004-at where the magnitude of carbon export was quantified based on the particles accumulation on the upward-facing window of vertically oriented transmissometers on autonomous platforms while they were drifting (i.e. parked at a specific depth). This method was later improved by [@Estapa2013-gw] who divided the attenuation signal into a continuous component, that is the continuous accumulation of small particles on the transmissometer window and a discontinuous component resulting in positive jumps likely attributed to the deposition of (rarer) large particles (@fig-estapa-method). @Estapa2017-jr further calibrated the OST carbon flux proxy using codeployed sediment traps and drifting transmissometers thus allowing the conversion of $c_{p}$ attenuation fluxes (units: m$^{-1}$d$^{-1}$) into carbon fluxes (units: mg C m$^{-2}$d$^{-1}$) using empirical POC-to-attenuance (ATN) ratio were ATN (unitless) is the product of $c_{p}$ and the transmissometer pathlength.
...
#jump_plot <- ggplot(clean_data, aes(x = dates, y = cp)) + geom_point() + geom_point(data = tmp2, aes(x = dates, y = cp, colour = colour))

# ggplot(despiked_data$data, aes(x = dates, y = cp)) + geom_step(size= 3, fill ='black') + 
#   geom_point(data = tmp2, aes(x = dates, y = cp, colour = colour), size = 3) + 
#   scale_colour_manual(values = c('black','yellow', 'blue')) +
#   theme_bw() +
#   ylab(TeX('$c_{p}~(m^{-1})$')) +
#   xlab('Datetime') +
#   theme(legend.position = c(0.095, 0.875),
#         legend.background = element_rect(fill = "white", color = "black"))

jump <- filter(tmp2, colour == 'jump')
...
# ggplot(despiked_data$data, aes(x = dates, y = cp)) + geom_point(size= 3, fill ='black') +
#   geom_point(data = jump, aes(x = dates, y = cp), size = 3, shape = 21, fill = '#DCEEF3') +
#   scale_colour_manual(values = c('black','yellow', 'blue')) +
#   theme_bw() +
#   ylab(TeX('$c_{p}~(m^{-1})$')) +
#   xlab('Datetime') +
#   theme(legend.position = 'top')
# 
# jump_plot <- ggplot(base_signal, aes(x = dates, y = cp)) + geom_point(size = 3, shape = 21, fill = '#003366') +
#   geom_point(data = jump, aes(x = dates, y = cp), size = 3, shape = 21, fill = '#DCEEF3') +
#   theme_bw() +
#   ylab(TeX('$c_{p}~(m^{-1})$')) +
#   xlab('Datetime')

#geom_path(data = part_slope, aes(x = time, y = cp, group = group), colour = 'red', size = 2) + theme_bw() + 
#labs(x = 'Time', y = 'Cp (1/m)', title = paste0('Mean slope (red): ', round(mean_slope,3), ' m-1 day-1')) # \n', 'POC flux (small particles): ', poc_flux, 'mg'))

jump_plot

```

The UVP6 (@fig-UVP6) is a miniaturized version of the UVP5 [@Picheral2021-sx], an imaging system developed to characterize particles (e.g. size, gray level, taxonomic group). Two versions of the UVP6 have been commercialized: the UVP6-HF (high frequency) and the UVP6-LP (low power) which has been specifically designed for AUVs that have strict energy requirements (e.g. autonomous floats).

The UVP6-LP (hereafter referred to as UVP6) is thus both a particle counter (0.1 mm - 2 mm) and a particle classifier (\> 0.6 mm). The former counts particles in different class sizes in order to compute the particle size distribution (PSD) - also referred to as PSS (@sec-BGP) - while the latter extracts pixel-related and geometric features from each image to classify them (i.e. assign them a taxonomic group) directly on board using artificial intelligence (CHAPTER-XX) with the aim of estimating zooplankton biomass [@Claustre2020-jo; @Claustre2021-tx].
...
While the gathering of data from automatic on board particles classification will have to increase to study related applications, the use of PSD in carbon flux studies has already been thoroughly explored [@Guidi2008-vp; @Iversen2010-an; @Jouandet2011-bt; @McDonnell2012-ic; @Roullier2014-wf; @Guidi2015-sj; @Guidi2016-nj; @Ramondenc2016-nu; @Kiko2017-qy; @Fender2019-ou; @Giering2020-mc; @Cael2021-qn; @Bisson2022-bt].

Using the PSD, one can compute the number spectrum $n_{i}$ [@Stemmann2004-jv] which can be defined as the normalized concentration of particles in a given size bin $i$. Knowing the mass ($m$) and sinking speed ($w$, see @eq-sinking-speed) of aggregates, one can derive the total carbon flux ($F$)
...
and therefore from @eq-mass-aggregate-fractal and @eq-sinking-speed-fractal that $A= \alpha\beta$ and $D = \frac{B+1}{2}$.
...
UVP6 PSD measurements will therefore allow the computation of carbon fluxes assuming that $A$ and $B$ are known. This was done empirically by @Guidi2008-vp who minimized the following cost function
...
## The future of carbon observations {#sec-what-is-next}

The BCP have been studied for many decades by a variety of instruments with their respective pros and cons. Their measurements were unfortunately often taken independently, which hindered their intercomparability due to the low number of concurrent measurements between instruments.

With the advent of fully-equipped AUVs, it is now possible to take high-resolution and independent measurements of key biogeochemical variables. Combining the latter with measurements from conventional platforms (i.e. ships, satellites, moorings) will fulfill the observational framework described by @Claustre2021-tx to investigate the BCP.

This framework will hopefully answer questions raised so far, that is better understand the underlying drivers of each component of the BCP, decouple the contribution of each pump to the observed downward carbon flux lengthily attributed to the BGP and better quantify global estimates of carbon storage.
