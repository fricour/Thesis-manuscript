...
The UVP6-LP (hereafter referred to as UVP6) is thus both a particle counter (0.1 mm - 2 mm) and a particle classifier (\> 0.6 mm). The former counts particles in different class sizes in order to compute the PSD while the latter extracts pixel-related and geometric features from each image to classify them (i.e. assign them a taxonomic group) directly on board using artificial intelligence (@sec-uvp6) with the aim of estimating zooplankton biomass [@Claustre2020-jo; @Claustre2021-tx].
...
2.  Four BGC-Argo floats equipped with both an OST and the UVP6 were deployed for the first time in the Labrador Sea in late May 2022 to study the dynamics of carbon fluxes and particle size distributions. In this chapter, I will explore the seasonality of the later and compare how the two sensors relate as two independent measurements of particle and carbon fluxes.
3.  Many studies have assumed that carbon sequestration only occurs in the deep ocean (typically 1000 m). In @sec-bcp-sequestration, we reassess the global sequestration ($\geq$ 100 years) of the BCP and its components by using a continuous approach to carbon storage as opposed to the common view of long-term sequestration below a fixed reference depth.
